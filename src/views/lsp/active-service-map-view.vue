...
export default {
  ...
  mounted() {
    ...
    if (multipleRoles.length > 1) {
      this.$refs.headercomp.createBreadcrumb([
        ...
        {
          text: this.$t("Map-View", "Map-View"),
          href: "/active-service-map-view",
        },
      ]);
    } else {
  ...
  methods: {
                  ...
                  if (localStorage.systemRole == 99) {
                    ...
                    if (i == noOfItems) {
                      ...
                      var region = new google.maps.LatLng(
                        currentVal.regionlocation[
                          currentVal.areaiterator
                        ].split(",")[0],
                        currentVal.regionlocation[
                          currentVal.areaiterator
                        ].split(",")[1]
                    ...
                    console.log(requestData.val().organizationKey);
                    console.log(localStorage.organizationKey);
                    if (
                      requestData.val().organizationKey.trim() ==
                      localStorage.organizationKey.trim()
                    ) {
                      var location = "";
                      location = requestData.val().location.split(",");
                      currentVal.regionlocation.push(
    ...
    showMapView: function (event) {
      Router.push("mactive-service-map-view");
    },
