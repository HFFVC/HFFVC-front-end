                        ...
                        v-if="
                          (user.email &&
                            user.email.trim().toLowerCase() !=
                              loggedInUserEmailId.trim().toLowerCase()) ||
                          (user.phoneNumber &&
              ...
              <div v-if="selectedUser">
                <div class="col col-md-12">
                  ...
                  <div class="row" v-if="phoneNumberEditable">
                    ...
                    <div class="inp-container phone-number">
                      <input
                  ...
                  <div class="row">
                    <div
                      v-if="
                        loggedInUserRole != undefined && loggedInUserRole == 99
                      "
                    >
                      <h4 class="accnt-details-heading">
                  ...
                  <div class="row">
                    <div class="user-roles-selection">
                      <label v-if="systemRole != undefined && systemRole == 96"
                        >Organization SPOC </label
                      ><br />
                      <div>
                        <input
...
export default {
  ...
  data: function () {
    return {
      ...
      userToDelete: null,
      isShowRole: false,
      loggedInUserRole: localStorage.systemRole,
      loggedInUserPhoneNumber: localStorage.phoneNumber || "",
      phoneNumber: null,
      loginMode: null,
      countryCode: null,
    ...
    if (localStorage.systemRole == 99) {
      if (getConfigVars().USERID == isAuthenticated()) {
        this.$refs.headercomp.createBreadcrumb([
          ...
          {
            text: this.$t("Manage LSP Users", "Manage LSP Users"),
            href: "/add-manage-user",
          },
        ]);
      } else {
        this.$refs.headercomp.createBreadcrumb([
  ...
  methods: {
    ...
    getUserList: function () {
      ...
      if (localStorage.systemRole.includes(99)) {
        ...
        ref.orderByChild("firstName").on("child_added", function (requestData) {
          ...
          if (
            requestData.val().userRoles != undefined &&
            requestData
              .val()
              .userRoles.some(
                (item) =>
          ...
          if (requestData.val().systemRole == 96 || isRolePresent) {
            const {
              email,
              firstName,
              lastName,
      ...
      if (this.pageTitle == this.$t("New User Details", "New User Details")) {
        ...
        console.log(
          firstName +
            ...
            email +
            "," +
            vm.userRolesSelected.length
        );

        if (firstName && lastName && vm.userRolesSelected.length > 0) {
        ...
        const ref = vm.unregisteredUser
          ? firebase.database().ref().child("phoneUsers").child(vm.snapshotKey)
          : firebase.database().ref().child("profile").child(vm.snapshotKey);

        ref
          .update(dataToUpdate)
          .then(() => {
            Vue.$toast.open({
              message: vm.$t("User details updated.", "User details updated."),
              type: "success",
              position: "top",
            });
            ...
            vm.clearAllFields();
            //   vm.$refs.radionButton.prop('checked', false);
    ...
    getOrganizationList() {
      ...
      ref.on("child_added", (requestData) => {
        if (
          requestData.val().status == undefined &&
          requestData.val().status != "disabled"
        ) {
    ...
    gotoDashboard: () => Router.push("lsp-dashboard"),
        ...
        .then((valid) => {
          ...
          firebase
            ...
            .then((snapshot) => {
              ...
              vm.seen = false;
              Vue.$toast.open({
                message: vm.$t("User data saved.", "User data saved."),
                type: "success",
                position: "top",
              });
              vm.clearAllFields();
    ...
    registerViaEmail() {
      ...
      firebase
        .auth()
        .createUserWithEmailAndPassword(vm.email, userPassword)
        .then((response) => {
          const snapshot = response.user.uid;
          sendMail(vm.email, `${vm.firstName} ${vm.lastName}`, userPassword)
            .then((sent) => {
              console.log(sent);
              const dataToPush = {
    ...
    getPhoneUsers() {
      ...
      firebase
        ...
        .on("child_added", (requestData) => {
          ...
          if (localStorage.systemRole.includes(99)) {
            vm.userList.push(dataToPush);
          } else {
            if (
              localStorage.organizationKey == requestData.val().organizationKey
            ) {
              vm.userList.push(dataToPush);
            }
          }
    ...
    updateExistingPhoneUser() {
      ...
      vm.ifPhoneNumberExists()
        .then((valid) => {
          console.log(valid);
          // delete user from profile node
          // vm.deleteOldUserRecord(dataToPush.uuid)
          //   .then((deleted) => {
          //     console.log(deleted);
