...
export default {
  ...
  mounted() {
    ...
    if (multipleRoles.length > 1) {
      this.$refs.headercomp.createBreadcrumb([
        ...
        {
          text: this.$t(
            "Previous Service Request - Map-View",
            "Previous Service Request - Map-View"
          ),
          href: "/previous-service-map/:id",
        },
      ]);
    } else {
      ...
      else
      {
        ...
        this.$refs.headercomp.createBreadcrumb([
          ...
          { text: "LSP-Dashboard", href: "/lsp-dashboard" },
          {
            text: this.$t(
              "Previous Service Requests",
              "Previous Service Requests"
  ...
  methods: {
    ...
    showMapView: function(event) {
      Router.push({ path: "/previous-service-map-view" });
    },
    showCalenderView: function(event) {
      Router.push({ path: "/previous-service-calender-view" });
    },
    showServiceRequests: function(event) {
      Router.push({ path: "/previous-service-requests" });
    },
